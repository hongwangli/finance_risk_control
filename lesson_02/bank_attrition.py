...
def NumVarPerf(df,col,target,pp, truncation=False):
    ...
    ax.margins(0.05) #左右边缘
    ax.set_ylim(bottom=0)
    pyplot.legend(loc='upper right') #图例位置
    pyplot.tight_layout()
    pp.savefig()
    pyplot.close(1)

def CharVarPerf(df,col,target,pp):
    '''
    ...
    '''
    validDf = df.loc[df[col] == df[col]][[col, target]] # remove NaN rows
    validRcd = validDf.shape[0]*1.0/df.shape[0]
...
bankChurn = pd.read_csv('http://oheum0xlq.bkt.clouddn.com/bankChurn.csv',header=0)
externalData = pd.read_csv('http://oheum0xlq.bkt.clouddn.com/ExternalData.csv',header = 0)
...
with PdfPages('multipage_pdf.pdf') as pdf:                                                                                          
    for i,var in enumerate(numericCols):
        NumVarPerf(AllData,var,'CHURN_CUST_IND',pdf)    

##need to do some truncation for outliers
with PdfPages('plot_file_percentile.pdf') as pdf2:
    for val in numericCols:
        NumVarPerf(AllData,val, 'CHURN_CUST_IND',pdf2,True)
#
#
##anova test
...
with PdfPages('categorical.pdf') as cpdf:
    for val in stringCols:
#        print val
        CharVarPerf(AllData,val,'CHURN_CUST_IND',cpdf)
